...
function explore(){
    ...
    for _child in $(hdfs dfs -ls "$path" 2>/dev/null | awk '{if(NF==8)print $8}'); do
        ...
        echo "$_child" "[$stat]"  | xargs -n 1 -I folder bash -c "f=\$(echo folder | awk '{print \$1}'); size=\$(hdfs dfs -du -s -h \$f 2>/dev/null | awk '{if(\$1!=0){print \$1\$2 } else print \"OM\"}' ); echo folder \$size" | sed -e 's/[^-][^\/]*\//--/g' -e 's/^/ /' -e 's/-/|/'
